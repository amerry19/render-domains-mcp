...
/**
 * render-domains-mcp — MCP server for Render custom domain management.
 *
 * Built to close the agent-loop gap in Render's official MCP, which exposes
 * read-only access to services/deploys/logs/metrics but no custom domain CRUD.
 *
...
const render = new RenderClient(token);
      ...
      return jsonContent({
        ...
        nextSteps: [
          domain.domainType === "apex"
            ? `Add an A record at @ pointing to 216.24.57.1 (or ALIAS/CNAME to ${render.constructor.name}-managed target)`
...
// ----------------------------------------------------------------------------
// DNS check (bonus — agent can confirm registrar DNS before triggering verify)
// ----------------------------------------------------------------------------
...
console.error("[render-domains-mcp] server ready (7 tools registered)");
