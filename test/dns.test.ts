...
import { RENDER_STATIC_SITE_A, renderDomainsDnsCheckLogic, type DohResolver } from "../src/dns.js";
...
describe("renderDomainsDnsCheckLogic", () => {
  it("reports pointsAtRender=true when A record matches the expected target", async () => {
    ...
    const result = await renderDomainsDnsCheckLogic({ domain: "example.com" }, resolver);
  ...
  it("reports pointsAtRender=true when CNAME ends with .onrender.com", async () => {
    ...
    const result = await renderDomainsDnsCheckLogic({ domain: "www.example.com" }, resolver);
  ...
  it("reports resolves=false when nothing comes back", async () => {
    ...
    const result = await renderDomainsDnsCheckLogic({ domain: "nothing.example.com" }, resolver);
  ...
  it("uses an explicit expectedTarget when provided", async () => {
    ...
    const result = await renderDomainsDnsCheckLogic(
